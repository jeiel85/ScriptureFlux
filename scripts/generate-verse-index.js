...
// 66권 순서 정보 (상위 books.json과 연동하기 위함)
const booksOrder = [
...
function generate() {
  ...
  for (let i = 0; i < booksOrder.length; i++) {
    const bookId = booksOrder[i];
    const chapterCounts = chapterVerseCountsData[bookId];
    if (!chapterCounts) {
      throw new Error(`Missing counts for book ${bookId}`);
    }

    const verseCount = chapterCounts.reduce((sum, val) => sum + val, 0);
    const startVerseOffset = totalVerses;

    books.push({
      bookIndex: i,
      startVerseOffset: startVerseOffset,
      verseCount: verseCount,
      chapterVerseCounts: chapterCounts
    });
  ...
  const outputDirSrc = path.resolve('src/data');
  const outputDirPub = path.resolve('public/data');

  if (!fs.existsSync(outputDirSrc)) {
    fs.mkdirSync(outputDirSrc, { recursive: true });
  }
  if (!fs.existsSync(outputDirPub)) {
    fs.mkdirSync(outputDirPub, { recursive: true });
  }

  const jsonContent = JSON.stringify(verseIndex, null, 2);
  fs.writeFileSync(path.join(outputDirSrc, 'verse-index.json'), jsonContent, 'utf-8');
  fs.writeFileSync(path.join(outputDirPub, 'verse-index.json'), jsonContent, 'utf-8');

  console.log(`Generated verse-index.json with ${totalVerses} total verses successfully!`);
}
