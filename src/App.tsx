        ...
        <section className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="flex-grow max-w-3xl">
            ...
            <p className="text-slate-400 leading-relaxed text-sm md:text-base">
              ScriptureFlux는 성경 66권 전체를 하나의 단일 유기적 축으로 배열하고, 구절 간의 긴밀한 상호 연관성을 수려한 2D 곡선 네트워크로 시각화합니다. 아래 곡선 위로 마우스 커서를 올려 연결된 구절과 그 텍스트를 실시간으로 확인해 보세요.
            </p>
          </div>
        ...
        <section className="relative">
          ...
          {!activeLink && (
            <div className="absolute top-4 left-4 glass-card px-3 py-1.5 rounded-lg border border-slate-800/50 text-[11px] text-slate-400 pointer-events-none animate-pulse">
              💡 곡선 네트워크에 마우스 커서를 올리거나 Hover 하세요.
            </div>
          )}
        </section>

        {/* 5. activeLink가 존재할 시 하단에 세부 구절 카드 정보 노출 */}
        <section className="min-h-[160px] flex items-center justify-center">
          {activeLink ? (
            <ReferenceCard activeLink={activeLink} />
          ) : (
            <div className="w-full text-center py-12 border border-dashed border-slate-800/50 rounded-2xl bg-slate-950/20 text-slate-500 text-sm">
              선택 또는 호버된 연결선이 없습니다. 캔버스 영역의 유기적 곡선에 마우스를 가져가 상세 정보를 확인하십시오.
            </div>
          )}
        </section>
      ...
      <footer className="border-t border-slate-900 bg-slate-950/80 py-6 text-xs text-slate-500">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap justify-center gap-x-6 gap-y-2">
            ...
            <span>
              Text: <a href="https://www.gutenberg.org" target="_blank" rel="noopener noreferrer" className="hover:text-emerald-400 transition-colors">KJV (Project Gutenberg Public Domain)</a>
            </span>
