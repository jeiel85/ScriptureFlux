...
### Changed
- **참조 연관 강도 필터 UI 직관성 개선**:
  - 모호했던 기존 '연관 강도(Weight) 필터' 명칭을 **'참조 연관 강도 필터'**로 다듬어 구절 간 참조에 관한 수치임을 직관적으로 인지하도록 개선.
  - 영문 축약형 'min. 0.10'을 한국어 사용자가 직관적으로 체감할 수 있는 **'최소 0.10'** 표기로 보완.
  - 슬라이더 바로 아래에 *"값이 높을수록 연관성이 깊은 핵심 연결선만 필터링하여 보여줍니다."*라는 미려하고 세련된 보조 설명 가이드를 추가하여 기능 이해도를 극대화.

### Build / CI
- `npm run lint` 및 `npm run typecheck`, Vite dist/ 번들 컴파일을 100% 무결하게 통과시켰습니다.

### Verification
- `npm.cmd run lint`: 통과
- `npm.cmd run typecheck`: 통과
- `npm.cmd run build`: 빌드 무오류 성공
