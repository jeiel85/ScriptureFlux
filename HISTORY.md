...
## 2026-05-26 (v0.10.1 - 참조 연관 강도 필터 UI 직관성 개선)

- 작업: 사용자가 '연관 강도(Weight) 필터 min. 0.10'이라는 기존 UI 표현을 더 직관적으로 이해할 수 있도록 용어를 '참조 연관 강도 필터' 및 '최소 {값}'으로 알기 쉽게 순화하고, 필터의 실질적인 기능과 작동 방식에 대한 친절한 보조 설명을 추가하여 UI 가독성을 극대화.
- 변경 파일:
  - `src/App.tsx`: 기존의 '연관 강도(Weight) 필터'를 '참조 연관 강도 필터'로 다듬고, 영문 표기 'min.'을 '최소'로 보완하여 국문 직관성을 개선. 또한 슬라이더 바로 아래에 "값이 높을수록 연관성이 깊은 핵심 연결선만 필터링하여 보여줍니다."라는 한 줄의 고급스러운 보조 캡션을 추가하여 기능의 직관성을 보완.
- 검증:
  - 로컬 `npm.cmd run typecheck`: TypeScript 형식 안전성 무경고 통과.
  - 로컬 `npm.cmd run lint`: ESLint 경고 및 오류 없이 통과.
  - 로컬 `npm.cmd run build`: Vite 정적 릴리즈 dist/ SPA 번들 1.90s 만에 컴파일 성공.
- 결과: 성공
